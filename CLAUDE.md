...
- **赏金 = 日常任务的升级版（执行层完全复用）**：接受后的赏金卡片加「▶ 开始计时 / ✓ 标记完成」按钮，和日常任务用**同一套** `TaskRunner` 计时、同一套 run 落地（`source="bounty"`）、同一套 `_calc_score` 得分、同样进时间轴和有效时间统计——零特殊逻辑。`TaskRunner` 加了 `source` + `onFinished(success)` props：计时成功完成后回调 `completeBounty()` 标记 done。直接勾选完成则写一条 `source=bounty` 的 run + `completeBounty()`。赏金区视觉仍独立（金色卡片 + buff 展示），只是获得了执行能力
